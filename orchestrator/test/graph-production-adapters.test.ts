...
describe("production adapter registry", () => {
  ...
  it("reconciles a settled absent Instagram image effect without requiring a fresh layout projection", async () => {
    ...
    const prior = runtime.engine.start({
      graphId: "deterministic-social-publication",
      version: "2.0.0",
      objective: "Prior settled absent Instagram image publication",
      input: {
        provider: "instagram",
        accountKey: "instagram:owner",
        expectedAccountId: "17841453638630920",
    ...
    runtime.store.saveRun({
      ...
      externalEffects: [{
        ...
        runId: prior.runId,
        nodeId: "publish_provider_object",
        idempotencyKey: "prior-instagram-absent",
        operationType: "production.instagram-publication-live.v2",
        target,
        payloadHash: "a".repeat(64),
        state: "ambiguous",
  ...
  it("leaves an unsettled Instagram image effect ambiguous without surfacing layout projection noise", async () => {
    ...
    const prior = runtime.engine.start({
      graphId: "deterministic-social-publication",
      version: "2.0.0",
      objective: "Prior unsettled Instagram image publication",
      input: {
        provider: "instagram",
        accountKey: "instagram:owner",
        expectedAccountId: "17841453638630920",
    ...
    runtime.store.saveRun({
      ...
      data: { publicationLive: { result: { outboxId } } },
      externalEffects: [{
        effectId: "gex_prior_instagram_unsettled",
        runId: prior.runId,
        nodeId: "publish_provider_object",
        idempotencyKey: "prior-instagram-unsettled",
        operationType: "production.instagram-publication-live.v2",
        target,
        payloadHash: "a".repeat(64),
        state: "ambiguous",
    ...
    const runner = {
      reconcileInstagramOutboxEntry: async () => ({
        ...
        entry: {
          ...
          kind: "image",
          status: "blocked",
          reconciliationClassification: "still_ambiguous",
          generatedMediaUploadCalls: 1,
          instagramPublishCalls: 1,
          browserRelayCalls: 0,
        },
