...
export const NON_GRAPH_RECURRING_WORK_REGISTRY: readonly NonGraphRecurringWorkRegistration[] = [
  ...
  {
    capability: "social-graph-maintenance",
    purpose: "Continuously reconcile stale Graph-owned social publication ambiguity before it blocks a later scheduled slot.",
    trigger: "Once at startup and setInterval every 5 minutes when the Graph runtime is enabled",
    owner: "orchestrator.service graph maintenance",
    state: "active",
    executionModel: "in-process maintenance interval -> Graph recovery plus canonical social runner reconciliation",
    effects: "Local Graph effect state, run recovery state, and expired capability state only.",
    idempotency: "An in-flight guard permits one sweep at a time; settled effects are consumed from canonical outbox/readback evidence.",
    concurrency: "Overlapping ticks are skipped while the previous sweep is still running.",
    recovery: "The timer is recreated on service startup and immediately performs a startup sweep.",
    verification: "Graph maintenance journal receipts, Graph effect states, canonical social outbox diagnostics, and zero-write counters.",
    reporting: "orchestrator journal and Graph health surfaces.",
    externalAuthority: "Read-only provider reconciliation only; no provider writes, Browser Relay calls, merges, deploys, or permission widening.",
    disposition: "REGISTERED_IN_PROCESS",
  },
  {
