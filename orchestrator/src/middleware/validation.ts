...
const ReviewSessionBucketSchema = z.enum([
  'baseline_idle',
  'startup_cost',
...
export const ReviewSessionBootstrapHandoffSchema = z.object({
  reviewSessionId: z.string().min(1).max(255),
  title: z.string().min(1).max(255),
  createdAt: z.string().datetime(),
  baselineStartedAt: z.string().datetime(),
  baselineEndedAt: z.string().datetime(),
  startupStartedAt: z.string().datetime(),
  machine: z.object({
    hostname: z.string().min(1).max(255),
    platform: z.string().min(1).max(120),
    arch: z.string().min(1).max(120),
    cpuModel: z.string().min(1).max(255),
    cpuCores: z.number().int().min(1).max(1024),
    memoryTotalMb: z.number().min(1).max(10_000_000),
  }),
  baselineSummary: z.object({
    cpuPercentAvg: z.number().min(0).max(100),
    cpuPercentPeak: z.number().min(0).max(100),
    loadAvg1m: z.number().min(0).max(10000),
    memoryUsedMbAvg: z.number().min(0).max(10_000_000),
    memoryUsedMbPeak: z.number().min(0).max(10_000_000),
  }),
  baselineSamples: z.array(z.object({
    capturedAt: z.string().datetime(),
    cpuPercent: z.number().min(0).max(100),
    loadAvg1m: z.number().min(0).max(10000),
    memoryUsedMb: z.number().min(0).max(10_000_000),
    memoryTotalMb: z.number().min(1).max(10_000_000),
  })).min(1).max(120),
  initialBucket: z.literal('startup_cost'),
  postHandoffBucket: ReviewSessionBucketSchema,
  capturePlan: z.object({
    profile: z.enum(['standard', 'soak-24h']),
    sampleIntervalMs: z.number().int().min(1000).max(86_400_000),
    maxSamples: z.number().int().min(60).max(100_000),
    intendedDurationHours: z.number().int().min(1).max(24 * 365).nullable().optional(),
    targetTaskCount: z.number().int().min(1).max(1_000_000).nullable().optional(),
  }),
  notes: z.array(z.object({
    capturedAt: z.string().datetime(),
    bucket: ReviewSessionBucketSchema,
    text: z.string().min(1).max(2000),
  })).max(50).optional().default([]),
});

export const ReviewSessionIdParamsSchema = z.object({
  id: z.string().min(1).max(255),
});

export const ReviewSessionBucketUpdateSchema = z.object({
  bucket: ReviewSessionBucketSchema,
  note: z.string().max(1000).optional(),
});

export const ReviewSessionNoteSchema = z.object({
  bucket: ReviewSessionBucketSchema,
  text: z.string().min(1).max(2000),
});

export const ReviewSessionLinkRunSchema = z.object({
  runId: z.string().min(1).max(255),
});

export const ReviewSessionExportQuerySchema = z.object({
