...
| `deployment-ops` | Yes | focused contract proof for bounded deployment posture synthesis | Yes | Yes | Expose now | Expose now as a deployment-governance lane. Keep the summary explicit about `ready`, `watch`, or `blocked` and do not overstate it as deploy authority. |
...
| `deployment-ops-agent` | `deployment-ops` | Present | focused bounded synthesis lane with deployment-governance contract proof | Summary only | Full readiness / evidence | Expose now | Best surfaced as a rollout-posture lane, not as a generic deploy button or hidden automation surface. |
