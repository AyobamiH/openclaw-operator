...
- only `doc-specialist` and `reddit-helper` are currently
  **service-expected**
- only `doc-specialist` and `reddit-helper` retain resident `src/service.ts`
  loops; worker-first lanes now rely on task-path evidence plus
  orchestrator-owned maintenance checks instead of synthetic service heartbeats
...
- `logs/*-service.json` is agent memory for every lane, and resident-service
  heartbeat truth only for `doc-specialist` and `reddit-helper`
