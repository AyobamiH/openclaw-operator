...
| `drift-repair` | `public-triggerable`; `confirmed working (2026-03-29 live repair refresh)` | `dynamic-only` | `driftRepairHandler` | `doc-specialist` | yes | Custom helper spawn path with manifest-bound `documentParser` preflight. The orchestrator watcher/queue is the sole mutation owner; `doc-specialist.service` remains observer-only. At most one logical drift repair may be pending, running, or retrying. Equivalent or expanded burst events are coalesced into the durable pending-path buffer; paths arriving after execution starts receive a distinct follow-up identity after the current run reaches its own truthful terminal state. Pending paths are recovered on restart. Exact same-run retries still require persisted recovery evidence. Knowledge-pack retention tolerates only an already-removed candidate (`ENOENT`) and propagates all other filesystem failures. A later successful repair never rewrites an earlier failed run. |
