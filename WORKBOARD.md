...
0. Full pre-Graph business-loop status is now stage-separated and dependency-addressable.
   ...
   - Graph-owned successor `campaign-content-factory-full-pregraph-v4` completed its first natural `05:00 Europe/London` trigger with one successful child, a passing verifier receipt, valid event chains, no recovery and zero provider writes
   - all 33 relevant natural Graph-owned cycles since PID start are terminal, their Graph and scheduler chains validate, and none produced a provider object; the Meta state-patch defect did not recur and no ambiguous effect was retried
   - official Instagram readback exposed a separate blocker: the represented account's exact reply existed before PID start, but all post-restart Meta reconciliation receipts classify it absent; duplicate containment prevented any new write
   - Phase 6 reports only supported official Threads metrics and leaves unsupported/CRM/website/conversation/conversion evidence unavailable; Phase 7 remains `READY_BUT_INTENTIONALLY_INACTIVE`; Phase 8 deterministic reports keep missing evidence visible
   - the immutable `15:00` Threads canary packet has no approval, rendered candidate or one-run live capability, and official readback confirms it was not published
   - terminal classification is `PRE_GRAPH_CONTRACT_GOVERNED_WITH_EXTERNAL_BLOCKERS`; rollback remains preserved at `20260808-full-pregraph-v3-cutover.UsbSaX`
